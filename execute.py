...
class Discretization:
    ...
    def execute(self):
        ...
        print("Best Split was on splitting point: ", self._best_split_dict["splitting_point"],
              " with an information-gain of: ", self._best_split_dict["_information_gain_for_current_split"],
              " and highest entropy of: ", self._best_split_dict["_net_entropy_for_current_split"])
        print("Best Split lte-data-points: ", self._best_split_dict["_lte_split_data_points"])
        print("Best Split gt_split_data_points: ", self._best_split_dict["_gt_split_data_points"])
    ...
    def _calculate_information_gain_of_whole_dataset(self):
        ...
        for unique_label in unique_labels:
            number_of_unique_label_data_points = self.dataset.where(
                self.dataset["label"] == unique_label
            ).dropna().data_points.count()

            probability_of_unique_label = number_of_unique_label_data_points / number_of_all_data_points
        ...
        self.information_gain_of_whole_dataset = -1 * sum(all_probabilities_with_their_log)
        print("information_gain_of_whole_dataset: ", self.information_gain_of_whole_dataset)
    ...
    def _compute_entropy_gains_of_all_splits(self):
        for _splitting_point_dict in self.splitting_points:
            ...
            for _label in self.dataset.label.unique():
                ...
                p_lte = df_lte.data_points.count() / _less_than_equal_to_splitting_point_df.data_points.count()
                ...
                p_gt = df_lte.data_points.count() / _greater_than_splitting_point_df.data_points.count()
                _all_probabilities_for_greater_than_with_their_log.append(
                    p_gt * calculate_log2(p_gt)
                )

            _entropy_for_lte = -1 * sum(_all_probabilities_for_less_than_equal_to_with_their_log)
            _entropy_for_gt = -1 * sum(_all_probabilities_for_greater_than_with_their_log)

            _probability_used_in_entropy_for_lte = _less_than_equal_to_splitting_point_df.data_points.count() / self.dataset.data_points.count()
            _probability_used_in_entropy_for_ge = _greater_than_splitting_point_df.data_points.count() / self.dataset.data_points.count()

            _net_entropy_for_current_split = (_probability_used_in_entropy_for_lte * _entropy_for_lte) + (
                    _probability_used_in_entropy_for_ge * _entropy_for_gt)

            _information_gain_for_current_split = self.information_gain_of_whole_dataset - _net_entropy_for_current_split
            print("..._information_gain_for_current_split: ", _information_gain_for_current_split)
            ...
            if _information_gain_for_current_split > self._greatest_gain_in_entropy:
                ...
                _splitting_point_dict["_information_gain_for_current_split"] = _information_gain_for_current_split
                _splitting_point_dict["_net_entropy_for_current_split"] = _net_entropy_for_current_split
                _splitting_point_dict["_lte_split_data_points"] = _less_than_equal_to_splitting_point_df.data_points.tolist()
                _splitting_point_dict["_gt_split_data_points"] = _greater_than_splitting_point_df.data_points.tolist()
